...
if ~exist(simfile,'file')
    ...
    V1mtx = nan(numel(V1vec),numel(V2vec));
    V2mtx = nan(numel(V1vec),numel(V2vec));
    for v1i = 1:numel(V1vec)
        V1 = V1vec(v1i);
        V1mtx(v1i,:) = ones(1,numel(V2vec))*V1;
        V2vec = flip(1./linspace(1/max([1,V1*5]+1),1,numel(V2vec))-1);
        V2mtx(v1i,:) = V2vec;
        parfor v2i = 1:numel(V2vec)
            V2 = V2vec(v2i);
            filename = sprintf('Test_V1_%1.1f_V2_%1.2f', V1, V2);
            if ~exist(fullfile(sim_dir, [filename, '.mat']), 'file')
                ...
                for t = 1:timesteps
                    ...
                    if t*dt <= 1600 && t*dt >= 50
                        InputSpikes(Ntwk.Input.Origins == 1) = InputSpikes(Ntwk.Input.Origins == 1) < spikeProbability*V1;
                        InputSpikes(Ntwk.Input.Origins == 2) = InputSpikes(Ntwk.Input.Origins == 2) < spikeProbability*V2;
                    else
                        InputSpikes(Ntwk.Input.Origins == 1) = 0;
                        InputSpikes(Ntwk.Input.Origins == 2) = 0;
                    end
                    % Synaptic activities
                    ...
                    ExctRefraction(Espikes) = refractionPeriod_E;
                ...
                fprintf('V1=%1.2f, V2=%1.2f Successfully completed\n', V1, V2);
                mysavefig(h1, filename, sim_dir, 12, [4, 4], 1);
                savefig(h1, fullfile(sim_dir,[filename, '.fig']));
                close(h1);
                h = figure; hold on;
            ...
            Modulation(v1i, v2i, :) = mean(s.FR(timep>600 & timep<1600,:),1);
        end
    end
    save(simfile, "Modulation","V1mtx","V2mtx");
else
    load(simfile);
end
...
for v1i = 1:numel(V1vec)
    V1 = V1vec(v1i);
    V2vec = flip(1./linspace(1/max([1,V1*3]+1),1,24)-1);
    plot(V2vec, Modulation(v1i,:,2), 'k.', 'Color', mygray(v1i+1,:), 'MarkerSize', 8);
end
legend({'V_1 = 0.1','V_1 = 1.0', 'V_1 = 5', 'V_1 = 50'},'Location','best');
ylabel('Mean firing rate (Hz)');
xlabel('V_2 (a.u.)');
mysavefig(h, filename, sim_dir, 12, [6, 2.5], 1);
...
% for v1i = 1:numel(V1vec)
%         V1 = V1vec(v1i);
%         V1mtx(v1i,:) = ones(1,24)*V1;
%         V2vec = flip(1./linspace(1/max([1,V1*3]+1),1,24)-1);
%         V2mtx(v1i,:) = V2vec;
% end
% V2vecp = flip(1./linspace(1/10,1,24)-1)
...
%     Rslts = [Rslts; new_row];
%     writetable(Rslts, fullfile(sim_dir, 'ModelFitRslts.txt'), 'Delimiter', '\t');
%     %%
...
%     ylabel('Mean firing rate (Hz)');
%     xlabel('V_2 (a.u.)');
%     mysavefig(h, filename, sim_dir, 12, [3, 2.5], 1);
% end
