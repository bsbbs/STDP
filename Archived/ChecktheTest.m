...
load(Seqfile);
Ntwkfile = fullfile(gnrloutdir, 'NtwkPrdcGPU_w0.1.mat');
load(Ntwkfile);
...
for evi = 1:numel(smplonsets)
    filename = sprintf('RealtimeMonitor_Event%i_w0.1', evi);
    ...
    lg = [];
    lg(1) = plot(timevec - min(timevec), Smpl.ExctgE(:,1), '-');
    lg(2) = plot(timevec - min(timevec), Smpl.ExctgI(:,1), '-');
    lg(3) = plot(timevec - min(timevec), Smpl.InhbtgE(:,1), '-');
    lg(4) = plot(timevec - min(timevec), Smpl.InhbtgI(:,1), '-');
    legend(lg, {'gE on E','gI on E', 'gE on I','gI on I'}, 'Location','best');
    xlabel('Time (ms)');
