...
plotdir = fullfile(gnrloutdir, 'SingleInputTestGPU');
...
Ntwk.Input.N = 400; % number of the input (excitarory) neurons
...
if show
    ...
    for i = 1:Ntwk.Input.Source
        plot(Ntwk.Input.Location(:,1), Ntwk.Input.Location(:,2), '.', 'Color', OKeeffe(i,:), 'MarkerSize', 3);
    end
    xlabel('\mum');
    ylabel('\mum');
...
InputSpikes = gpuArray.rand(Ntwk.Input.N, timesteps) < spikeProbability*Seq;
