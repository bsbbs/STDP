...
plotdir = Runningdir; %fullfile(Projdir, 'SyncSmallerPatch_dt0.1');
Srchfile = fullfile(plotdir, 'Seq*.mat');
D = dir(Srchfile);
Seqfile = fullfile(D.folder, D.name);
load(Seqfile);
...
for evi = 0:numel(smplonsets)
    if evi == 0
        ...
        filename = sprintf('Weights_Seg%i', evi);
