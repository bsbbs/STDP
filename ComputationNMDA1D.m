...
dt = .1;
Ntrial = 1600;
ProjectName = sprintf('SyncSmallerPatch_dt0.1');
...
% test the network
V1vec = 0:.5:2;
V2vec = flip(1./linspace(1/10,1,24)-1);
simfile = fullfile(plotdirintersect, 'ModulationEffect.mat');
    ...
    for v1i = 1:numel(V1vec)
        ...
        parfor v2i = 1:numel(V2vec)
            ...
            if ~exist(fullfile(plotdirintersect_sim, [filename, '.mat']), 'file')
                ...
                save(fullfile(plotdirintersect_sim, [filename, '.mat']), "-fromstruct", s);
                Modulation(v1i, v2i, :) = mean(FR(timep>600 & timep<1600,:),1);
            else
