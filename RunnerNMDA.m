...
Smpl.InhbtgNMDA = [InhbtgAMPA(smplI,1)'; nan(smplsteps-1, numel(smplI))];
...
for t = 1:(timesteps-1)
    ...
    if smpl && mod(t*dt, smplintrvl) == 0
        ...
        Smpl.ExctgNMDA(smplt,:) = EgNMDAi(smplE)';
    ...
    if smpl
        ...
        figure(h2);
        plot(Ntwk.Exct.Location(Espikes,1), Ntwk.Exct.Location(Espikes,2),'w.', 'MarkerSize', 4);
        plot(Ntwk.Inhbt.Location(Ispikes,1), Ntwk.Inhbt.Location(Ispikes,2),'r.', 'MarkerSize', 5);
        if mod(t*dt, 1) == 0
