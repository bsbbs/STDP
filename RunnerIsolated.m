function [WEE, WEI, WIE] = RunnerIsolated(Ntwk, Seq, dt, Runningdir)
...
for t = 1:(timesteps-1)
    ...
    if smpl
        if showspikes
            figure(h1); hold on;
            plot(t*dt/1000*ones(sum(Espikes),1), find(Espikes), 'k.', 'MarkerSize', 8);
            plot(t*dt/1000*ones(sum(Ispikes),1), find(Ispikes)-.1, 'r.', 'MarkerSize', 10);
        end
