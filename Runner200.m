function [WEE, WEI, WIE] = Runner200(Ntwk, Seq, dt, Runningdir)
...
InputxNMDAi = zeros(Ntwk.Input.N, 1);
InputgNMDAi = zeros(Ntwk.Input.N, 1);
ExctgAMPA = zeros(Ntwk.Exct.N, 1);
ExNMDAi = zeros(Ntwk.Exct.N, 1);
EgNMDAi = zeros(Ntwk.Exct.N, 1);
ExctgNMDA = zeros(Ntwk.Exct.N, 1);
ExctgGABA = zeros(Ntwk.Exct.N, 1);
InhbtgAMPA = zeros(Ntwk.Inhbt.N, 1);
IxNMDAi = zeros(Ntwk.Exct.N, 1);
IgNMDAi = zeros(Ntwk.Exct.N, 1);
InhbtgNMDA = zeros(Ntwk.Inhbt.N, 1);
InhbtgGABA = zeros(Ntwk.Inhbt.N, 1);
...
InhbtNoise = Ib + randn(Ntwk.Inhbt.N,1)*Ntwk.Noise.sgm; % OU noise on inhibitory neurons
...
smplsteps = (500+200)/smplintrvl - 1;
timevec = nan(smplsteps,1);
...
Smpl.WEI = zeros(smplsteps, numel(smplI), numel(smplE));
Smpl.WEI(1,:,:) = WEI(smplI, smplE);
Smpl.WIE = zeros(smplsteps, numel(smplE), numel(smplI));
Smpl.WIE(1,:,:) = WIE(smplE, smplI);
Smpl.WEE = zeros(smplsteps, numel(smplE), numel(smplE));
...
if video
    h2 = figure('Position', [1, 1, 1280, 460]); hold on;
    pbaspect([2 1 1]);
...
for t = 1:(timesteps-1)
    ...
    Evis = (t*dt - Seq.evs*1000) >= 0 & (t*dt - Seq.evs*1000) <= 500;
    InputSpikes = rand(Ntwk.Input.N,1);
    ...
    WEI(WEI<0) = 0;
    % Overall constraints on the synaptic strength (C - sum(WEI,2)).*
    ...
    InhbtNoise = InhbtNoise + ((Ib - InhbtNoise)/Ntwk.Noise.tauN + randn(Ntwk.Inhbt.N,1)*Ntwk.Noise.sgm)*dt;
    ...
    % Example traces
    if any(t == smplonsets - 99/dt)
        Sgi = find(t == smplonsets - 99/dt);
        if showspikes
            figure(h1);
            xlim([(smplonsets(Sgi)*dt+[-100, 1300])/1000]);% 0 Ntwk.Exct.N+1]);
            cla;
        end
        if video
            ...
            disp(profiles);
            writerObj1 = VideoWriter(fullfile(Runningdir, filename), 'Motion JPEG AVI');
    ...
    if smpl
        if showspikes
            figure(h1); hold on;
            plot(t*dt/1000*ones(sum(Espikes),1), Ntwk.Exct.Location(Espikes,1), 'k.', 'MarkerSize', 8);
            plot(t*dt/1000*ones(sum(Ispikes),1), Ntwk.Inhbt.Location(Ispikes,1), 'r.', 'MarkerSize', 10);
        end
        if showweights && mod(t*dt, 50) == 0
            figure(h3); hold on;
            plot(t*dt/1000, WEE(smplE(1), smplE(2)), '.', 'Color', [.5,.5,.5], 'MarkerSize', 5);
            plot(t*dt/1000, WEI(smplI(1),smplE(1)), 'b.', 'MarkerSize', 8);
            plot(t*dt/1000, WIE(smplE(1), smplI(1)), 'r.', 'MarkerSize', 10);
            plot(t*dt/1000, WEI(smplI(1),smplE(2)), 'b.', 'MarkerSize', 8);
            plot(t*dt/1000, WIE(smplE(1), smplI(2)), 'r.', 'MarkerSize', 10);
            plot(t*dt/1000, WEI(smplI(2),smplE(1)), 'b.', 'MarkerSize', 8);
            plot(t*dt/1000, WIE(smplE(2), smplI(1)), 'r.', 'MarkerSize', 10);
            plot(t*dt/1000, WEI(smplI(2),smplE(2)), 'b.', 'MarkerSize', 8);
            plot(t*dt/1000, WIE(smplE(2), smplI(2)), 'r.', 'MarkerSize', 10);
        end
        if video
            figure(h2);
            plot(Ntwk.Exct.Location(Espikes,1), Ntwk.Exct.Location(Espikes,2),'w.', 'MarkerSize', 8);
            plot(Ntwk.Inhbt.Location(Ispikes,1), Ntwk.Inhbt.Location(Ispikes,2)-1,'r.', 'MarkerSize', 10);
            if mod(t*dt, 1) == 0
    ...
    if any(t == smplonsets + 1300/dt)
        % s = struct("WEE", WEE, "WEI", WEI, "WIE", WIE);
        Sgi = find(t == smplonsets + 1300/dt);
        ...
        if showspikes
            figure(h1);
            filename = sprintf('SpikeTrain_Seg%i', Sgi);
            % axis([(smplonsets(Sgi)*dt+[-100, 1300])/1000 0 Ntwk.Exct.N+1]);
            % mysavefig(h1, filename, Runningdir, 12, [4, 4], 1);
            savefig(h1, fullfile(Runningdir,[filename, '.fig']));
        end
        if showweights
            figure(h3);
            filename = sprintf('WeightsDynmc');
            savefig(h3, fullfile(Runningdir,[filename, '.fig']));
        end
        if video
            figure(h2);
            cla;
